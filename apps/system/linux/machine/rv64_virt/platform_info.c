...
extern struct remoteproc_ops rv64_virt_linux_proc_ops;
...
platform_create_proc(int proc_index, int rsc_index)
{
	...
	if (!remoteproc_init(&rproc_inst, &rv64_virt_linux_proc_ops,
			     &rproc_priv))
		return NULL;
	...
	memcpy(rsc_table, get_resource_table(0, &rsc_len), rsc_len);
